<x-layouts.app :title="__('Dashboard')">
    ...
    <!-- Filter Tanggal -->
    <div
        class="mb-4 bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 p-6">
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-neutral-900 dark:text-white mt-1">Rp
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-neutral-900 dark:text-white mt-1">
                        {{ number_format($totalKendaraan) }}</p>
                </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-neutral-900 dark:text-white mt-1">
                        {{ number_format($kapasitasTersedia) }} / {{ number_format($totalKapasitas) }}</p>
                </div>
            ...
            <div class="mt-3">
                <div class="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2">
                    <div class="bg-orange-600 h-2 rounded-full"
                        style="width: {{ $totalKapasitas > 0 ? min(($totalTerisi / $totalKapasitas) * 100, 100) : 0 }}%">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-3">
                    @forelse($statistikPerPetugas as $petugas)
                        <div
                            ...
                            <span
                                class="text-sm font-medium text-neutral-700 dark:text-neutral-300">{{ $petugas->name }}</span>
                            <span
                                class="text-sm font-bold text-blue-600 dark:text-blue-400">{{ $petugas->total_transaksi }}
                                transaksi</span>
                        </div>
                    @empty
                    ...
                    @forelse($statistikArea as $area)
                        <div class="p-3 bg-neutral-50 dark:bg-neutral-700/50 rounded-lg">
                            <div class="flex justify-between items-center mb-2">
                                <span
                                    class="text-sm font-medium text-neutral-700 dark:text-neutral-300">{{ $area->nama_area }}</span>
                                <span
                                    class="text-xs text-neutral-500 dark:text-neutral-400">{{ $area->terisi }}/{{ $area->kapasitas }}</span>
                            </div>
                            <div class="flex justify-between text-xs">
                                <span class="text-neutral-600 dark:text-neutral-400">{{ $area->total_transaksi }}
                                    transaksi</span>
                                <span class="text-green-600 dark:text-green-400 font-semibold">Rp
                                    {{ number_format($area->pendapatan_area ?? 0, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @empty
                        <p class="text-sm text-neutral-500 dark:text-neutral-400 text-center py-4">Belum ada area
                            parkir</p>
                    @endforelse
                </div>
            </div>
    ...
    @else
        ...
        <div
            class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 p-6 mb-4">
            <h3 class="text-lg font-semibold text-neutral-900 dark:text-white mb-4">Statistik Per Area Parkir</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                @empty
                    <p class="text-sm text-neutral-500 dark:text-neutral-400 text-center py-4 col-span-full">Belum ada
                        area parkir</p>
                @endforelse
            </div>
        </div>
    ...
    <div
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead
                    class="bg-neutral-50 dark:bg-neutral-900/50 border-b border-neutral-200 dark:border-neutral-700">
                    <tr>
                        <th
                    ...
                    @forelse($transaksiTerbaru as $index => $transaksi)
                        <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                            ...
                            <td
                                ...
                                {{ $transaksi->kendaraan->plat_nomor }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400">
                                {{ $transaksi->area->nama_area }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400">
                                {{ $transaksi->waktu_masuk->format('d M Y, H:i') }}</td>
                            ...
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold text-green-600 dark:text-green-400">
