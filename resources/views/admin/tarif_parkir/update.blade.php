<x-layouts.app :tittle="__('Edit Tarif Parkir')">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 gap-4">
        <div>    
            <flux:heading size="xl">Edit Tarif Parkir</flux:heading>
            <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                Perbarui informasi tarif parkir.
            </p>
        </div>
    ...
    <div class="mt-4 bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 p-6">
        <form action="{{ route('admin.tarif-parkir.update', $tarifParkir) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div>
                    ...
                    <select name="jenis_kendaraan" id="jenis_kendaraan" 
                        ...
                        <option value="mobil" {{ old('jenis_kendaraan', $tarifParkir->jenis_kendaraan) == 'mobil' ? 'selected' : '' }}>Mobil</option>
                        <option value="motor" {{ old('jenis_kendaraan', $tarifParkir->jenis_kendaraan) == 'motor' ? 'selected' : '' }}>Motor</option>
                        <option value="lainnya" {{ old('jenis_kendaraan', $tarifParkir->jenis_kendaraan) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           step="0.01"
                           min="0"
                           value="{{ old('tarif_per_jam', $tarifParkir->tarif_per_jam) }}"
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mt-8 pt-6 border-t border-neutral-200 dark:border-neutral-700">
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Terakhir diperbarui: {{ $tarifParkir->updated_at->format('d M Y, H:i') }}
                </p>
